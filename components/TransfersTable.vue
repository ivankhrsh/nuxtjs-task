    ...
    <div class="overflow-hidden rounded-row bg-portal-primary-light-100 px-2">
      <div class="hidden overflow-x-auto sm:block">
        <div class="min-w-[820px]">
          <div
            class="grid grid-cols-[152px_1fr_1fr_1fr_1fr_180px] items-center gap-8 px-4 py-[10px] text-sm font-medium tracking-body text-on-surface-variant"
          ...
          >
            <div class="flex items-center gap-2 text-sm leading-5">
              <span class="whitespace-nowrap font-medium tracking-body text-on-surface">{{ transfer.date }}</span>
              <span class="whitespace-nowrap tracking-body-md text-on-surface-variant">{{ transfer.time }}</span>
            </div>
            <span class="text-sm leading-5 tracking-body-md text-on-surface">{{ transfer.type }}</span>
            <span class="text-sm leading-5 tracking-body-md text-on-surface">{{ transfer.method }}</span>
            <span class="font-nums text-body-md leading-5 tracking-body-md text-on-surface">{{ transfer.accountMasked }}</span>
            <span class="font-nums text-right text-body-md font-medium leading-5 tracking-body text-on-surface">{{ transfer.amount }}</span>
            <div class="flex justify-center">
              <span
                class="inline-flex rounded px-2 py-[2px] text-xs font-medium leading-4 tracking-body-md"
      ...
      <div class="flex flex-col sm:hidden">
        ...
        >
          <div class="mt-3 grid grid-cols-[auto_1fr] gap-x-6 gap-y-[10px] text-sm leading-5">
            <span class="tracking-body text-on-surface-variant">Date</span>
            <div class="flex items-center gap-2">
              <span class="font-medium tracking-body text-on-surface">{{ transfer.date }}</span>
              <span class="tracking-body-md text-on-surface-variant">{{ transfer.time }}</span>
            </div>
            <span class="tracking-body text-on-surface-variant">Type</span>
            <span class="tracking-body-md text-on-surface">{{ transfer.type }}</span>
            <span class="tracking-body text-on-surface-variant">Method</span>
            <span class="tracking-body-md text-on-surface">{{ transfer.method }}</span>
            <span class="tracking-body text-on-surface-variant">Account</span>
            <span class="font-nums text-body-md tracking-body-md text-on-surface">{{ transfer.accountMasked }}</span>
            <span class="tracking-body text-on-surface-variant">Amount ($)</span>
            <span class="font-nums text-body-md font-medium tracking-body text-on-surface">{{ transfer.amount }}</span>
            <span class="tracking-body text-on-surface-variant">Status</span>
            <span>
              <span
                class="inline-flex rounded px-2 py-[2px] text-xs font-medium leading-4 tracking-body-md"
                :class="statusClasses[transfer.status]"
      ...
      <div v-if="!formattedRows.length" class="flex flex-col items-center gap-3 py-6">
        ...
        <p class="m-0 text-center text-sm leading-5 tracking-body-md text-on-surface-variant">
